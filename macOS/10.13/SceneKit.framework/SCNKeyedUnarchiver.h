/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/SceneKit.framework/Versions/A/SceneKit
 */

@interface SCNKeyedUnarchiver : NSKeyedUnarchiver {
    SCNAssetCatalog * _assetCatalog;
    NSDictionary * _context;
    NSURL * _documentURL;
    id  lookUpFoundInstance;
    NSString * lookUpKey;
}

@property (nonatomic, readwrite, retain) SCNAssetCatalog *assetCatalog;
@property (nonatomic, readwrite, retain) NSDictionary *context;
@property (nonatomic, readonly) NSURL *documentEnclosingURL;
@property (nonatomic, readwrite, retain) NSURL *documentURL;
@property (nonatomic, readwrite, retain) id lookUpFoundInstance;
@property (nonatomic, readwrite, retain) NSString *lookUpKey;

- (id)assetCatalog;
- (id)context;
- (void)dealloc;
- (id)documentEnclosingURL;
- (id)documentURL;
- (id)lookUpFoundInstance;
- (id)lookUpKey;
- (void)setAssetCatalog:(id)arg1;
- (void)setContext:(id)arg1;
- (void)setDocumentURL:(id)arg1;
- (void)setLookUpFoundInstance:(id)arg1;
- (void)setLookUpKey:(id)arg1;

@end
