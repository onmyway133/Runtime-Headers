/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/CoreImage.framework/Versions/A/CoreImage
 */

@interface CIPortraitLightingFront : CIFilter {
    CIVector * inputBottom1;
    CIVector * inputBottom2;
    NSNumber * inputBrighten;
    CIVector * inputCenter1;
    CIVector * inputCenter2;
    NSNumber * inputContrast;
    NSNumber * inputHeight1;
    NSNumber * inputHeight2;
    CIImage * inputImage;
    NSNumber * inputRotate1;
    NSNumber * inputRotate2;
    NSNumber * inputStrength;
    NSNumber * inputWidth1;
    NSNumber * inputWidth2;
}

@property (nonatomic, readwrite, retain) CIVector *inputBottom1;
@property (nonatomic, readwrite, retain) CIVector *inputBottom2;
@property (nonatomic, readwrite, retain) NSNumber *inputBrighten;
@property (nonatomic, readwrite, retain) CIVector *inputCenter1;
@property (nonatomic, readwrite, retain) CIVector *inputCenter2;
@property (nonatomic, readwrite, retain) NSNumber *inputContrast;
@property (nonatomic, readwrite, retain) NSNumber *inputHeight1;
@property (nonatomic, readwrite, retain) NSNumber *inputHeight2;
@property (nonatomic, readwrite, retain) CIImage *inputImage;
@property (nonatomic, readwrite, retain) NSNumber *inputRotate1;
@property (nonatomic, readwrite, retain) NSNumber *inputRotate2;
@property (nonatomic, readwrite, retain) NSNumber *inputStrength;
@property (nonatomic, readwrite, retain) NSNumber *inputWidth1;
@property (nonatomic, readwrite, retain) NSNumber *inputWidth2;

+ (id)customAttributes;

- (id)_dualLightKernel;
- (id)inputBottom1;
- (id)inputBottom2;
- (id)inputBrighten;
- (id)inputCenter1;
- (id)inputCenter2;
- (id)inputContrast;
- (id)inputHeight1;
- (id)inputHeight2;
- (id)inputImage;
- (id)inputRotate1;
- (id)inputRotate2;
- (id)inputStrength;
- (id)inputWidth1;
- (id)inputWidth2;
- (id)outputImage;
- (void)setInputBottom1:(id)arg1;
- (void)setInputBottom2:(id)arg1;
- (void)setInputBrighten:(id)arg1;
- (void)setInputCenter1:(id)arg1;
- (void)setInputCenter2:(id)arg1;
- (void)setInputContrast:(id)arg1;
- (void)setInputHeight1:(id)arg1;
- (void)setInputHeight2:(id)arg1;
- (void)setInputImage:(id)arg1;
- (void)setInputRotate1:(id)arg1;
- (void)setInputRotate2:(id)arg1;
- (void)setInputStrength:(id)arg1;
- (void)setInputWidth1:(id)arg1;
- (void)setInputWidth2:(id)arg1;

@end
