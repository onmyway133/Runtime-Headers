/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/CFNetwork.framework/Versions/A/CFNetwork
 */

@interface NSHost : NSObject {
    NSArray * addresses;
    NSArray * names;
    id  reserved;
}

@property (atomic, readonly, copy) NSString *address;
@property (atomic, readonly, copy) NSArray *addresses;
@property (atomic, readonly, copy) NSString *localizedName;
@property (atomic, readonly, copy) NSString *name;
@property (atomic, readonly, copy) NSArray *names;
@property (nonatomic, readwrite, retain) __NSHostExtraIvars *reserved;

+ (id)currentHost;
+ (void)flushHostCache;
+ (id)hostWithAddress:(id)arg1;
+ (id)hostWithName:(id)arg1;
+ (BOOL)isHostCacheEnabled;
+ (void)setHostCacheEnabled:(BOOL)arg1;

- (void)__resolveWithFlags:(int)arg1 resultArray:(id)arg2 handler:(id)arg3;
- (id)_thingToResolve;
- (id)address;
- (id)addresses;
- (void)blockingResolveUntil:(int)arg1;
- (void)dealloc;
- (id)description;
- (id)initToResolve:(id)arg1 as:(int)arg2;
- (BOOL)isEqualToHost:(id)arg1;
- (id)localizedName;
- (id)name;
- (id)names;
- (id)reserved;
- (void)resolve:(id)arg1;
- (void)resolveCurrentHostWithHandler:(id)arg1;
- (void)setReserved:(id)arg1;

@end
